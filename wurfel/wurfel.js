...
function fitCanvasToScreen() {
  ...
  const cssScale = size / BASE_CANVAS_SIZE;
  p5Canvas.style('transform', `translateZ(0) scale(${cssScale})`);
  p5Canvas.style('will-change', 'transform');
  ...
  if (mouse && mouse.mouseConstraint && mouse.mouseConstraint.mouse) {
    const scale = BASE_CANVAS_SIZE / size;
    mouse.mouseConstraint.mouse.pixelRatio = 1;
    Matter.Mouse.setScale(mouse.mouseConstraint.mouse, { x: scale, y: scale });
  }
}
...
function setup() {
  ...
  if (p5Canvas && p5Canvas.elt) {
    ...
    p5Canvas.elt.style.backfaceVisibility = 'hidden';
  ...
  mouse = new Mouse(engine, p5Canvas, {
    stroke: 'magenta',
    strokeWeight: 2
  });
