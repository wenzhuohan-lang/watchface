...
// --- 设备重力（手机：deviceorientation） ---
let deviceGravityEnabled = false;
let deviceGamma = 0; // 左右倾斜 (-90..90)
let deviceBeta = 0;  // 前后倾斜 (-180..180)
let smoothGamma = 0;
let smoothBeta = 0;
const DEVICE_SMOOTH = 0.12; // 平滑系数，越小越平滑但更迟滞
const DEVICE_GRAVITY_MAG = 0.8; // 重力强度（与 world.gravity.y 原来的值一致）
...
function setup() {
  ...
  // 在移动设备上首次触碰或点击时尝试请求 deviceorientation 权限（iOS 需要）
  ...
  // 只在第一次触摸/点击时请求（避免重复弹窗）
  window.addEventListener('touchstart', requestDeviceOrientationPermission, { passive: true, once: true });
  window.addEventListener('click', requestDeviceOrientationPermission, { once: true });
...
function handleDeviceOrientation(e) {
  // e.gamma: 左右倾斜，范围大致 -90..90
  // e.beta: 前后倾斜，范围大致 -180..180（但通常 -90..90 用于手机）
...
function draw() {
  ...
  // --- 重力：优先使用设备重力（手机），否则使用键盘/手动设置（gravityX/Y） ---
  if (deviceGravityEnabled) {
    // 平滑设备倾角，减少抖动
    smoothGamma = lerp(smoothGamma, deviceGamma, DEVICE_SMOOTH);
    smoothBeta  = lerp(smoothBeta,  deviceBeta,  DEVICE_SMOOTH);

    // 将倾角归一化到 [-1,1]（gamma / 90, beta / 90）
    const nx = constrain(smoothGamma / 90, -1, 1);
    const ny = constrain(smoothBeta  / 90, -1, 1);

    // 把倾角方向映射为重力方向（保持恒定强度 DEVICE_GRAVITY_MAG）
    const len = Math.hypot(nx, ny);
    if (len < 0.02) {
      // 接近平放设备时，保持向下默认重力
      world.gravity.x = 0;
      world.gravity.y = DEVICE_GRAVITY_MAG;
    } else {
      world.gravity.x = DEVICE_GRAVITY_MAG * (nx / len);
      world.gravity.y = DEVICE_GRAVITY_MAG * (ny / len);
    }
  } else {
    // 回退：使用键盘/手动设置（与原逻辑相同）
    let gx = gravityX;
    ...
    if (gx !== 0 || gy !== 0) {
      ...
      world.gravity.x = DEVICE_GRAVITY_MAG * gx / len;
      world.gravity.y = DEVICE_GRAVITY_MAG * gy / len;
    } else {
      world.gravity.x = 0;
      world.gravity.y = DEVICE_GRAVITY_MAG; // 标准：向下
    }
  }
